...
public interface PrimeChecker {
    ...
    static PrimeChecker newInstance() throws RuntimeException {
        return ServiceLoader.load(PrimeChecker.class)
                .findFirst()
                .orElseThrow(() -> new RuntimeException("No PrimeChecker service provider found."));
    }
    ...
    static PrimeChecker newInstance(String providerName) throws RuntimeException {
        ...
        throw new RuntimeException("A PrimeChecker service provider with the name '" + providerName + "' was not found.");
    }

    static List<PrimeChecker> startsWith(String prefix) {
        return ServiceLoader.load(PrimeChecker.class)
                .stream()
                .filter((ServiceLoader.Provider p) -> p.type().getName().startsWith(prefix))
